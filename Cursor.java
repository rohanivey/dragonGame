...
public class Cursor {
	...
	public void setTexture() {
		// TODO: Find some way to get a copy of a texture so I can break links
		// if need be
	...
	public void swapItem(Slot inputSlot) {
		...
		System.out
				.println("Cursor.swapItem() now has a tempItem with the value of "
						+ tempItem.getInputString());
		// Removes item we just got a copy of
		inputSlot.getIM().removeItem(inputSlot);
		System.out
				.println("Cursor.swapItem() has removed the original item in the inputSlot");
		// Add the item from main hand to slot
		inputSlot.getIM().addItem(inputSlot, heldItem);
		System.out
				.println("Cursor.swapItem() now has added the item it held into the InventoryManager at the slot");
		// Swap item in our off hand to main hand
		heldItem = tempItem;
		System.out
				.println("Cursor.SwapItem() now holds tempItem as its current heldItem");
		System.out
				.println("Cursor.swapItem() how has a heldItem with a value of "
						+ heldItem.getInputString());
		setTexture();
	...
	public Boolean testOwner(Slot inputSlot) {
		...
		if (slotOwner.equals(itemOwner))
			return true;
		// If the slot is owned by an entity and the item isn't owned by the
		// player
	...
	public Boolean testSwap(Slot inputSlot) {
		...
		System.out
				.println("Cursor.testSwap() is creating a copy of the inventory involved with the swap");
		...
		System.out
				.println("Cursor.testSwap() how holds an item in tempSlot. The item is named: "
						+ tempSlot.getCurrentItem().getInputString());

		// Play with the copies
		tempIM.removeItem(tempSlot);
		System.out
				.println("Slot.testSwap() has now removed the item in tempSlot");
		// Is there room for the currently held item at the location of the item
		// we just temporarily removed
		if (tempIM
				.checkGridRoom(heldItem, tempSlot.getRow(), tempSlot.getCol())) {
