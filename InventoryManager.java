...
public class InventoryManager {
	...
	public InventoryManager(Entity inputEntity) {
		System.out
				.println("Setting up the Inventory Manager for an entity in the InventoryManager constructor");
	...
	public InventoryManager(int inputGH, int inputGW, int inputSX, int inputSY,
			Slot[][] originalGrid, ownerType inputType, Cursor inputCursor) {
		gridHeight = inputGH;
		gridWidth = inputGW;
		startingX = inputSX;
		startingY = inputSY;
	...
	public InventoryManager(Player inputPlayer) {
		System.out
				.println("Setting up the Inventory Manager for the player in the InventoryManager constructor");
		player = inputPlayer;
	...
	public InventoryManager(TradeHandler inputTH) {
		System.out
				.println("Setting up the Inventory Manager for the Tradehandler in the InventoryManager constructor");
	...
	public Boolean addItem(Item inputItem) {
		for (int row = 0; row < gridHeight; row++) {
			for (int col = 0; col < gridWidth; col++) {
				// If the scan finds an unused grid it will proceed to the next
				// step
				if (grid[row][col].getCurrentType().equals("UNUSED")) {
					// If there's room for the item at the grid location this
					// will return true and the item will be inserted
					...
					if (checkGridRoom(inputItem, row, col)) {
						...
						// inputItem.getInputString() + " to slot [" + row +
						// ", " + col + "] in inventoryManager.addItem()");
	...
	public Boolean checkGridRoom(Item inputItem, int inputRow, int inputCol) {
		...
		// System.out.println("Item " + inputItem.getInputString() + " is " +
		// inputItem.getGridWidth() + " tiles wide and " +
		// inputItem.getGridHeight() + " tiles high.");
		// System.out.println("Grid is beginning at location [" + inputRow +
		// ", " + inputCol + "]");
		...
		for (int row = inputRow; row <= itemHeight && row < grid.length; row++) {
			...
			for (int col = inputCol; col <= itemWidth && col < grid[row].length; col++) {
				// If the grid is clear as it checks the length/width of the
				// item in the rows/colums available
				if (grid[row][col].getCurrentType().equals("UNUSED")) {
					colClear += 1;
					// Broke up the below for debug purposes.
					// If the total check turns out that all dimensions needed
					// are clear, then return true; the item can be added
					if (rowClear == inputItem.getGridHeight()) {
						// System.out.println("Row was clear at " + rowClear +
						// " rows tall clear, checking column");
						if (colClear == inputItem.getGridWidth()) {
							// System.out.println("Column was clear at " +
							// colClear +
							// " columns wide clear, returning true");
							// If every number has been checked and returned as
							// Unused, then we're all clear and proceed with the
							// process
							return true;
						}
			...
			if (colClear == 0) {
				// If none of the blocks are clear on this row of columns, set
				// rowClear back to 0 to show a break in storage
				rowClear = 0;
				// System.out.println("Checking next row, col did not return a valid destination for the item.");
			}
		}

		// There wasn't enough room for the item at the specified coordinates,
		// so return false
		System.out
				.println("InventoryManager.checkGridRoom() didn't find enough room for the item");
		return false;
	}

	public Slot[][] copyGrid(Slot[][] inputGrid) {
		System.out.println("InventoryManager.copyGrid() is beginning");
		System.out.println("InventoryManager.copyGrid() inputGrid is "
				+ inputGrid.length + " by " + inputGrid[0].length);
		// There won't be ragged arrays so assuming [0].length is the same as
		// [15555].length is fine here
		Slot[][] tempGrid = setupGrid();
		System.out.println("InventoryManager.copyGrid() tempGrid is "
				+ tempGrid.length + " by " + tempGrid[0].length);
		System.out
				.println("InventoryManager.copyGrid() has established tempGrid's[][]");
		...
		for (int row = 0; row < tempGrid.length; row++) {
			for (int col = 0; col < tempGrid[row].length; col++) {
				// CHECK TO SEE IF THE SLOT IS SLAVE OR MASTER, not if there is
				// an item there.
				// TODO: CHECK THIS FOR VERIFICATION
				if (inputGrid[row][col].getCurrentItem() != null) {
					System.out
							.println("InventoryManager.copyGrid() is trying to read the value at "
									+ row + ", " + col);
					System.out
							.println("InventoryManager.copyGrid() inputGrid has an item named "
									+ inputGrid[row][col].getCurrentItem()
											.getInputString());
					Item tempItem = inputGrid[row][col].getCurrentItem()
							.copyMe();
					System.out
							.println("InventoryManager.copyGrid() has an established tempItem with the name "
									+ tempItem.getInputString());
					tempGrid[row][col].setItem(tempItem);
					System.out
							.println("InventoryManager.copyGrid() tempGrid now has an item named "
									+ tempGrid[row][col].getCurrentItem()
											.getInputString());
					// Need to tell the slot to become a master and tell this im
					// that the new slots around it are slaves
					tempGrid[row][col].setType("MASTER");
					// Enslave the slots for this item
					System.out
							.println("Enslaving slots in InventoryManager.pickUpItem");
					enslaveSlots(tempGrid[row][col].getCurrentItem(), row, col);
					System.out.println("Adding copied item "
							+ tempGrid[row][col].getCurrentItem()
									.getInputString() + " at location:[" + row
							+ ", " + col + "]");
				}
	...
	public void enslaveSlots(Item inputItem, int inputRow, int inputCol) {
		...
		for (int i = inputRow; i < maxHeight; i++) {
			for (int j = inputCol; j < maxWidth; j++) {
				// Set every free slot previously checked by checkGridRoom() to
				// a slave status
				...
				if (i == inputRow && j == inputCol) {
					...
					// Debug info
					// System.out.println("The slot at coords " + i + ", " + j +
					// " is now enslaved to slot [" + inputRow + ", " + inputCol
					// + "].");
				}
				// Tell every slot assigned as a slave that its new master slot
				// is at grid inputX, inputY
	...
	public void removeItem(Slot inputSlot) {
		// Setting the slot to an unused slot allows for handling all the
		// dropping item, setting slot to unused, freeing its slaves, etc
	...
	public Slot[][] setupGrid() {
		...
		for (int row = 0; row < gridHeight; row++) {
			for (int col = 0; col < gridWidth; col++) {
				...
				grid[row][col] = slot;
				// System.out.println("Slot's owner is " +
				// getType().toString());
				// System.out.println("Adding new slot at location:[" + row +
				// ", " + col + "]");
			}
		}
		return grid;
